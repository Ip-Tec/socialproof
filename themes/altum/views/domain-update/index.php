...
<?php endif ?>
    ...
    <div class="d-flex justify-content-between">
        <h1 class="h4 text-truncate"><i class="fas fa-fw fa-xs fa-globe mr-1"></i> <?= l('domain_update.header') ?></h1>

        <?= include_view(THEME_PATH . 'views/domains/domain_dropdown_button.php', ['id' => $data->domain->domain_id]) ?>
    </div>
    <div class="alert alert-secondary mb-4"><?= sprintf(l('domains.help'), '<strong>' . (settings()->notifications->domains_custom_main_ip ?: $_SERVER['SERVER_ADDR']) . '</strong>', '<strong>' . $host . '</strong>') ?></div>
