...
<?php ob_start() ?>
    ...
    <div class="form-group" data-file-input-wrapper-size-limit="<?= settings()->notifications->audio_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->notifications->audio_size_limit) ?>">
        <label for="audio"><i class="fas fa-fw fa-sm fa-eye text-muted mr-1"></i> <?= l('notification.settings.audio') ?></label>
        <?= include_view(THEME_PATH . 'views/partials/file_input.php', ['uploads_file_key' => 'notifications_audios', 'file_key' => 'audio', 'already_existing_file' => $data->notification->settings->audio]) ?>
        <?= \Altum\Alerts::output_field_error('audio') ?>
        <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::get_whitelisted_file_extensions_accept('notifications_audios')) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->notifications->audio_size_limit) ?></small>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                ...
                id="settings_audio_autoplay"
                name="audio_autoplay"
            <?= $data->notification->settings->audio_autoplay ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="settings_audio_autoplay"><?= l('notification.settings.audio_autoplay') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="settings_audio_controls"
                name="audio_controls"
            <?= $data->notification->settings->audio_controls ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="settings_audio_controls"><?= l('notification.settings.audio_controls') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="settings_audio_loop"
                name="audio_loop"
            <?= $data->notification->settings->audio_loop ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="settings_audio_loop"><?= l('notification.settings.audio_loop') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="settings_audio_muted"
                name="audio_muted"
            <?= $data->notification->settings->audio_muted ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="settings_audio_muted"><?= l('notification.settings.audio_muted') ?></label>
    </div>
...
<script>
    ...
    $('#settings_title').on('change paste keyup', event => {
        $('#notification_preview .altumcode-audio-title').text($(event.currentTarget).val());
    });

    $('#settings_audio').on('change paste keyup', event => {
        if($('#notification_preview audio')) {
            $('#notification_preview audio').attr('src', $(event.currentTarget).val());
        }
    });
    ...
    $('#settings_button_text').on('change paste keyup', event => {
        ...
        if(text.trim() == '') {
            $('#notification_preview .altumcode-audio-button').hide();
        } else {
            $('#notification_preview .altumcode-audio-button').show().text(text);
        }
    });
    ...
    settings_title_color_pickr.on('change', hsva => {
        ...
        $('#notification_preview .altumcode-audio-title').css('color', hsva.toHEXA().toString());
    });
    ...
    settings_button_background_color_pickr.on('change', hsva => {
        ...
        /* Notification Preview Handler */
        $('#notification_preview .altumcode-audio-button').css('background', hsva.toHEXA().toString());
    });
    ...
    settings_button_color_pickr.on('change', hsva => {
        ...
        /* Notification Preview Handler */
        $('#notification_preview .altumcode-audio-button').css('color', hsva.toHEXA().toString());
    });
