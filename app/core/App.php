...
class App
{
    ...
    public function __construct()
    {
        ...
        if (in_array(\Altum\Router::$controller, ['PixelTrack', 'PixelWebhook'])) {
        ...
        if (is_logged_in()) {
            ...
            if ($user->plan_settings->notifications_impressions_limit != -1 && $user->current_month_notifications_impressions > $user->plan_settings->notifications_impressions_limit && !isset($_COOKIE['plan_notifications_impressions_limit_notice'])) {
                Alerts::add_warning('<strong>' . l('global.notifications.user_notifications_impressions_limit.title') . '</strong><br />' . sprintf(l('global.notifications.user_notifications_impressions_limit.description'), '<a href="' . url('plan') . '" class="font-weight-bold">', '</a>'));
        ...
        /* Maintenance mode */
        if (
            isset(settings()->main->maintenance_is_enabled) &&
            settings()->main->maintenance_is_enabled &&
            (!is_logged_in() || $user->type != 1) &&
