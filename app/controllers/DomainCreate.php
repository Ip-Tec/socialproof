...
class DomainCreate extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(in_array($_POST['host'], settings()->notifications->blacklisted_domains)) {
                Alerts::add_field_error('host', l('campaigns.error_message.blacklisted_domain'));
            }

            if(!empty($_POST['custom_index_url']) && in_array(get_domain_from_url($_POST['custom_index_url']), settings()->notifications->blacklisted_domains)) {
                Alerts::add_field_error('custom_index_url', l('campaigns.error_message.blacklisted_domain'));
            }

            if(!empty($_POST['custom_not_found_url']) && in_array(get_domain_from_url($_POST['custom_not_found_url']), settings()->notifications->blacklisted_domains)) {
                Alerts::add_field_error('custom_not_found_url', l('campaigns.error_message.blacklisted_domain'));
            }
